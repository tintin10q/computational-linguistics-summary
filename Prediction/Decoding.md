...
In the case of this course decoding has to do with [Hidden Markov Models](Hidden%20Markov%20Models.md.md). It is the task of **determining the sequence of hidden variables given a sequence of observed events.** 
...
So the apoximated tag sequence is the sequence of words with the highest posterior probabilty. If you actually calculate this with [Bayes rule](Bayes%20rule.md.md) you get $\hat{t}_{1:n} =\text{argmax}_{t_{1:n}} p(t_{1:n}) \cdot p(W_{1:n})$. 
We plug in the formulas to estimage both terms under the [Markov assumption and the output indepedence assumption](Hidden%20Markov%20Models.md.md). So for a whole sequence:
